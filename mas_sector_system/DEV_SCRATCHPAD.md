...
| FWD-01a | Driver templates for `general` and `software_saas` only → `driver_templates.py`. Structure must let the other 14 slot in without an API change. | Gemini → Claude/Opus-5 | DONE — shipped by Gemini, corrected + tested on takeover: §8.1 clamps, §8.2 asymmetric margin band, `OUTPUT_KINDS` for all 16, §7 per-segment name resolution. `drivers_for()` now returns `(drivers, is_native)`; the remaining 14 are pure data inserts from here |
...
### [2026-07-30] - [Claude/Opus-5] - [FWD-01a: Driver template corrections + tests (picked up from Gemini)]
- **What I changed:** Took over the Track A lane — Gemini is off the roster. Four corrections to `driver_templates.py`, none of them cosmetic.
  1. **Hard clamps now match §8.1.** The shipped template allowed `segment_growth` up to **10.0** (1000% revenue growth), `opex_growth` to **5.0**, and `gross_margin` to **1.0**. The guardrail table says −0.50/2.00, −0.30/1.50 and 0.0/0.99. A clamp that never binds is not a clamp, and this is the §2 "few tight dials" control the whole epic rests on.
  2. **`gross_margin` band restored to the spec's asymmetry.** §8.2 is `[hist_min − 1000bps, hist_max + 500bps]`; both templates had symmetrised it (±0.10 general, ±0.05 saas). The asymmetry is deliberate — margin may be argued down further than up, and symmetrising loosens the ceiling, which is the direction that inflates a forecast.
  3. **`OUTPUT_KINDS` for all 16 archetypes**, straight from the §5.2 output column. `forecast_output_kind()` previously returned `eps_fcf` for every archetype without a template, so a bank, an insurer or a REIT silently reported an EPS/FCF forecast. Output *shape* is doctrine and is knowable today; only the driver *sets* are held on FWD-07, so this needed no baseline.
  4. **Per-segment driver names now resolve to a band.** §7 names revenue drivers `revenue_growth.data_center`; the engine also emits `segment_growth_1`. Neither found the template's `segment_growth` entry, so a per-segment driver had no band and the §8.2 dissent check had nothing to test.
- **API change (announced, not unilateral):** `drivers_for()` now returns `(drivers, is_native)` rather than a bare list, mirroring `valuation_doctrine.exemplar_block_for()`. §8's fallback row requires the no-template case to run consolidated-only **and flagged**; it previously returned `[]` with no flag, so a substituted set was indistinguishable from a native one. It now returns the `general` drivers plus `is_native=False`. Returned dicts are deep copies — a caller mutating one no longer corrupts the module-level template for the rest of the process. `band_for_driver()` and `forecast_output_kind()` keep their spec signatures. **Nothing imported this module, so no caller broke.** The design doc's API block is annotated.
- **Files modified:** `mas_sector_system/driver_templates.py`, `tests/test_driver_templates.py` (new, 19 tests), `mas_sector_system/FORWARD_ESTIMATE_DESIGN.md` (API-block annotation only — §7 untouched), `mas_sector_system/DEV_SCRATCHPAD.md`.
- **Notes / Handoff for next agent:** Four things.
  1. **The template and the engine still do not touch.** `forecast_engine.py` imports nothing from here — it hard-codes `gross_margin` / `opex_growth` and reads segment growth from `segment_growth` / `revenue_growth.*` keys directly. Until it calls `drivers_for()`, the template is documentation. Logged as FWD-11.
  2. **`build_forecast()` would reject a `software_saas` driver set built from its own template.** The template names `sm_margin` / `rd_margin` per §5.2; the engine hard-requires `opex_growth` and raises without it. That is an unimplemented archetype path in the engine (Track B), **not** a defect in the template — do not "fix" it by bending the template back to the general set, or the SaaS dials disappear.
  3. **`build_forecast()` never checks `forecast_output_kind()`.** It will build an EPS/FCF forecast for a REIT if asked. The output map now exists to gate on; the gate itself is engine-side.
  4. **§8.1 has no entry for `sm_margin` / `rd_margin`.** I left Gemini's values (0.0–2.0 and 0.0–1.0) rather than invent spec numbers, but a 200%-of-revenue hard ceiling on S&M is loose enough that it will never bind. Worth a spec decision when FWD-01b lands, not a silent edit.
- **Verification:** `python3 -m pytest tests/ -q` → **207 passed** (188 + 19 new). The financials test asserts the exact regression: `bank_lender` → `residual_income`, `insurance` → `book_value`, both REITs → `ffo`, `mortgage_reit` → `book_value`, with no template present for any of them.
- **Status:** COMPLETED

### [2026-07-30] - [Claude/Opus-5] - [FWD-01a addendum: where the clamps and bands actually live]
- **What I changed:** No behaviour change. Verified the claim in the entry above against the running code, and corrected one test that was asserting the wrong ceiling.
- **Files modified:** `tests/test_driver_templates.py` (`test_driver_records_are_well_formed` split; entry-count guard now models segment expansion), `mas_sector_system/DEV_SCRATCHPAD.md`.
- **Notes / Handoff for next agent:** Three findings, and the second is a trap.
  1. **§8.1 has no competing table.** `valuation_engine.ARGUED_INPUT_BOUNDS` covers `wacc`, `g_terminal`, `g_high`, `high_growth_years`, `fade_years`, `justified_multiple` — valuation parameters, zero overlap with the forecast drivers. So `driver_templates.py` is the only home for the §8.1 forecast clamps and there is no drift hazard between two tables. Confirmed, not assumed.
...
### [2026-08-01] - [Claude/Opus-5] - [FWD-07-FIX: engine arithmetic + grader + disclosure fixes]
- **What I changed:** Independent review of Grok's FWD-07 baseline, then fixed what it surfaced. Full write-up in `FWD07_REVIEW.md` (repo root) — 14 issues, ~11 tracing to 5 root causes. Commit `5f385a8` on branch `fix/valuation-engine-arithmetic` (off `fwd-07-baseline`). Every corrected number was hand-computed independently before touching code, then reproduced by the fix.
- **Files modified:** `valuation_engine.py`, `valuation_rubric.py`, `artifacts.py`, `memory.py`, `agents.py`, `tests/test_memory.py`; new `tests/test_argued_arithmetic_invariants.py`, `tests/test_rubric_c11_scoping.py`. **`driver_templates.py` / `test_driver_templates.py` deliberately left unstaged** — that is in-flight FWD-01a work, not mine.
- **Two blocker bugs (both shipped wrong numbers into memos):**
  1. **Sensitivity deltas measured across two base-FCF bases.** Bit KO and QCOM (argued `base_fcf_method` ≠ engine basis); NVDA/CRM were clean. KO's memo said *"WACC is the dominant lever… g_high barely matters (−$1.47)"* — truth is g_high −$18.78 (dominant, downward) and WACC +$12.81. Its `DIRECTIONAL BIAS: 98% above default` disclosure should not have fired at all (2 material args, 59% share, downward). Fixed via `neutral_case`; `base_fcf_method` now gets its own row (it was KO's largest lever, +$17.31, previously invisible).
  2. **Corners not sign-aware.** Index [0]/[1] of each range let low-WACC (optimistic) cancel low-g_high (pessimistic). KO's `low_case` returned the *higher* value; band was 3.1× too narrow ($5.55 vs true $17.10) while disclosed as "wider than the analysis supports". Ends now chosen by measured direction — probed empirically, because `high_growth_years`/`fade_years` only raise value while `g_high > g_terminal`. Gordon clamp still runs after selection.
- **Delivery gap:** `collect_valuation_disclosures` only read `dcf_engine`/`comps_engine`, so **every** judgment-layer disclosure was dropped — no DIRECTIONAL BIAS, clamp, inert-FCF or band-dissent line reached any audit log in the whole eight-ticker run. Epic V's control layer was firing into a void. Now wired. **Note for Epic F: `artifacts.py` and `pdf_generator.py` reference none of `historical_profile` / `forecast` / `driver_critique` / `dcf_modelled` / `consensus_reconciliation`. Wire each field to an artifact as part of its own ticket or Epic F ships invisible too.**
- **Grader fixes (re-graded on the committed slices — no re-run, no API spend):** C11 3/8→8/8, C3 2/8→4/8, C5 7/8→**4/8**. The C5 drop is the fix working: JPM/PGR were passing on a hardcoded ±15% band and PLD on a null range. C3's 4 remaining fails are the 8 figures that genuinely appear nowhere in state (JPM `$900B`/`$118B`/`$4.8T`, PGR `$95B`/`$5.1B`, PLD `$5.5B`/`$235B`, QCOM `$3.2B`) — those are real and worth a hard stop.
- **`eps` removed from C11.** Annual vs quarterly vs annualized-quarterly cannot be separated by regex — the distinction is in the sentence. JPM/PGR/PLD were all failed wrongly; PLD had tagged each figure with its canonical metric id. Correct check is traceability (C3's shape), not contradiction. Not built — flagged in the review.
- **Tests:** 219 → 235. The two new invariants are proven to fail against the old behaviour, not just to pass against the new.
- **Ownership note:** `valuation_rubric.py` is not in the module table in §Ownership; VAL-13/15 history makes it effectively Grok's. Edited on explicit user instruction — Grok, please review the C3/C5/C11 changes rather than assume them.
- **Not fixed — deliberately routed (see review §5, §6, §9, §11):**
  1. **@Codex — FWD-01b, non-FCF judgment layer.** JPM/PLD/PGR now define the shape: argued params are inert, sensitivities empty, and the ±15% band is cosmetic (and now correctly fails C5). PLD additionally needs FFO/NAV to populate a fair value at all. You own `validate_argued_inputs` + the argued corners from VAL-10, so this is your ground.
  2. **@Grok — clean re-run on a fixed harness template, and FWD-06.** The current baseline is not a controlled experiment: prompts range 113→245 chars and JPM/PLD/PGR are *told* which method to use, while C1 grades whether the method was justified. Separately, `load_prior_run` has no mode filter, so the JPM/PLD/PGR/KO re-runs ingested their own throwaway `valuation_only` first pass (rows 21–23 → 24–27 in `research_memory.sqlite`) and wrote a Thesis Evolution section against a phantom. Re-run needs one prompt template + memory isolated. **This is what FWD-01b/FWD-03 should consume — not the current numbers.** For FWD-06, review §4b: nothing in the rubric checks whether a valuation is *plausible* or whether the recommendation follows from the primary method. KO scored 10/11 while its DCF said the stock was 72% overvalued and the memo said HOLD.
  3. **XOM is entity resolution, not commodity extraction.** SEC maps XOM → CIK `0002115436` "ExxonMobil Holdings Corp" — a July 2026 holdco reorg, 26 filings all `8-K`/`8-K12B` from one week, `formerNames: []`, zero XBRL history. `get_cik_for_ticker` never checks the resolved entity *has* filings. It hard-stopped by luck; a holdco with one 10-Q would have produced a confident memo on one quarter. **Swap in CVX/COP to unblock `cyclical_commodity` now** and file entity resolution separately — fixing XOM teaches nothing about commodity drivers.
  4. **C8 is LLM-judged and flips between runs.** It drives 3 of 8 scores. Either pin it and record a distribution or split the headline into mechanical (9) + judged (2, advisory).
- **Housekeeping:** the FWD-07 handoff says the artifacts/harness are gitignored. `outputs/` and `tmp/` *are* in `.gitignore`, but commit `aba7d6b` force-added all 24 — `git ls-files` confirms they are tracked. Decide which you want and make them agree.
- **Status:** COMPLETED (fixes); routed items above are open.

### [2026-08-01] - [Codex/GPT-5] - [FWD-01b: non-FCF judgment layer]
- What I changed: Extended the deterministic argued-input layer to residual-income and FFO/NAV methods. Added evidence-gated hard clamps for cost of equity, sustainable/fade ROE, plowback, FFO multiple, cap rate, and NAV discount; the same validator now resolves forecast-driver hard clamps lazily from `driver_templates`. Residual-income cases now recompute ROE fade, book-value growth, and discounting. Equity-REIT cases now produce a fair value from observable FFO/P-FFO anchors with explicit multiple, cap-rate, and NAV-discount cases. Every reported range is the true min/max over all range-end combinations, with empirical sign probes and one-at-a-time sensitivities. Legacy 2026-07-30 critiques are translated only where their reasoning has an unambiguous native meaning (`wacc` → cost of equity; `g_high` book growth → plowback).
...
### [2026-08-02] - [Claude/Opus-5] - [Review of FWD-01b (Codex) + FWD-07-CLEAN/FWD-06 (Grok)]
- **What I changed:** Reviewed both commits, fixed two defects found, verified the rest against the clean-run slices. Commits `c2468ae`, `6f48924`. Suite 257→**263 pass, 0 fail** (was 1 failing on arrival).
- **Verified working — the end-to-end proof we did not have before.** KO's regenerated memo now reads: *"the sensitivity analysis is unambiguous that the base-FCF choice is the dominant driver — moving from `engine_base_fcf` to `mid_cycle` alone adds +$30.18/share."* The lever that was invisible before FWD-07-FIX is now named as dominant, with the right inference drawn from it. The false `DIRECTIONAL BIAS: 98% above default` disclosure is gone. Compare against the shipped 2026-07-30 memo, which said "WACC is the dominant lever… g_high barely matters (−$1.47)". Both engine bugs are confirmed dead on live output, not just in unit tests.
- **Codex FWD-01b — good work.** JPM/PGR/PLD now produce genuinely computed asymmetric ranges from real non-FCF dials (`cost_of_equity`, `plowback`, `ffo_multiple`, `cap_rate`, `nav_discount`); PLD has a fair value where it had `None`; the `fade_years clamped from 15 to 10` disclosure flows; the zero-delta and true-extremes invariants were extended to the non-FCF paths and run (no skips).
- **Grok FWD-07-CLEAN — good work.** One prompt template, `prior_run_id=None` on all eight, `MAS_MEMORY_DISABLE_PRIOR` + scratch DB, XOM→CVX, mechanical/advisory split, `fwd_vacuous: 7` reported separately so F1–F7 inflate nothing. F-plausibility/F-coherence tests fail in both directions — the criteria are not decorative. `recommendation` added to `state.py` as suggested.
- **DEFECT FIXED 1 — silent failure (`c2468ae`).** `_residual_income_case` / `_ffo_nav_case` guarded on missing inputs by returning the base unchanged: `equity_value=None`, `errors=[]`, and `_excess_return_on_equity` then appended "Residual-income model is simplified…" implying it had run. The engine computed nothing and reported success. Caught by the pre-existing `test_valuation_dispatch_never_fcf_for_hard`, which was **failing on arrival**. Both now record a reason; residual income computes equity value without a share count (only per-share needs it). Third invariant added: a case yields a number or a reason, never neither. No regression — JPM/PGR/PLD still value on the real slices.
- **DEFECT FIXED 2 — C5 loophole (`6f48924`).** The FFO/NAV fallback builds its range from constants the code picks (±2 turns, ±75bp, ±10pt). Nonlinear blending makes it asymmetric, so `_is_mechanical_band` misses it, and PLD passed C5 with every sensitivity at zero delta. `range_basis` ("argued" | "convention") now rides the judgment block; convention ranges do not satisfy C5. The fallback range is still computed and disclosed — only its C5 pass is removed. **Stored slices predate the flag, so PLD's C5 flips on the next run, not retroactively.**
- **OPEN — @Codex, FWD-01b is half done.** The engine consumes native non-FCF dials, but **nothing proposes them.** On Grok's fresh 2026-08-02 run the critique layer still argued pure FCF dials at a bank, an insurer and a REIT:
  - JPM: `wacc, g_high, g_terminal, high_growth_years, fade_years, base_fcf_method`
  - PGR: `wacc, g_terminal, high_growth_years, fade_years, base_fcf_method`
  - PLD: `base_fcf_method, g_terminal, wacc, g_high, high_growth_years, fade_years`
...
### [2026-08-03] - [Claude/Opus-5] - [TEST-06: offline harness + VAL-07: calibration]

- **What I changed:** Built the two items ranked highest in `REVIEW_2026-08-03.md` §3 — the offline end-to-end harness (F6) and the outcome scorecard (the gap behind "the rubric grades process, not results"). Commits `1ae9fe4`, `279aa25`. Suite **288 → 325 pass, 0 fail**, ~8s, no network.
- **Files added:** `mas_sector_system/offline.py`, `mas_sector_system/calibration.py`, `tests/test_offline_end_to_end.py` (9), `tests/test_calibration.py` (28). No existing module changed, so no lane was crossed.

**TEST-06 — the graph now runs without spending anything.**
`offline_mode()` swaps the model and every network seam for recorded material. Routing, deferred joins, validation, the valuation engine, `artifacts.py`, `cost.py` and `memory.py` are deliberately **not** patched — they are what is under test. `_invoke` stays intact (retry-on-empty, usage extraction, `record_llm_call` all still run); only `_llm` returns a fake client, with **fixed zero token counts** so cost assertions don't drift when a fixture is edited.

1. **Transcript sources are complementary, and both are needed.** Statements + engine blocks come from a `*_state_slice_fwd_clean.json`; agent prose comes from `research_memory.sqlite`. **`bull_thesis` / `bear_thesis` are empty on every stored slice** — the slice writer never captured them — and the DB has them for runs 24–27. `transcript_from_slice(..., memory_db=...)` merges the two. @Grok: worth adding both fields to the slice writer if you re-run.
2. **Gaps are reported, not swallowed.** `Transcript.missing` lists labels that fell back to placeholder text. This matters because the placeholder is short enough to trip `_is_weak_output`, which silently doubles that node's call count — the first run showed `bull: 2, bear: 2` purely from this, which would have made "each node ran once" meaningless.
3. **What the nine tests actually prove** — none of this was reachable before: every node fires exactly once (the multi-parent fan-in guard, checked by counting real invocations rather than reading edges); validation FAIL halts before capital/bull/bear/synthesis/qc and still writes the audit log with no .docx; QC FAIL hard-stops with no .docx and the log still written; the export path writes all three artifacts and exactly one cost line; a weak response is retried exactly once. TEST-04 and TEST-07 are effectively covered by these.
4. **The harness leaked on its first run** — a .docx and a cost line landed in the real `outputs/`. Both removed. `offline_deep_dive` now redirects `artifacts.DEFAULT_OUTPUT_DIR`, `export_docx.DEFAULT_OUTPUT_DIR` and `cost.DEFAULT_COST_LOG`, and **raises** if a redirect target has been renamed rather than letting a run write beside real deliverables. `MAS_MEMORY_DB` points at a scratch file. Verified after a full suite run: `outputs/` unchanged, cost log still 40 lines, memory still 26 runs.
5. **What it cannot do:** it exercises the machine, not the judgment. It says nothing about whether a memo is any good. Real runs are still the only test of that — just far fewer of them.

**VAL-07 — the desk can now score itself.**
`valuation_rubric.py` grades *how an argument was made*; all eleven criteria are process. Nothing asked whether the call was right. `calibration.py` does.

1. **Direction is primary, target accuracy secondary.** Most calls carry no target, and post-VAL-18 `null` is the *correct* value when the memo issues none — a target-keyed scorer would be structurally empty. BULLISH is right if the stock rose through the band, BEARISH if it fell through, NEUTRAL if it stayed inside (a HOLD predicts no move and earns credit when that happens). Band defaults to ±5% and is a CLI dial.
2. **Coverage is a first-class output.** An empty scorecard prints **no hit rate at all** rather than 0% — a rate without its denominator is the number that gets a desk in trouble.
3. **Nothing fetches a price.** The lookup is injected, so scoring is pure and runs offline; `yfinance_prices` is one implementation and `--prices file.json` is the offline route. **All numbers I produced used invented prices** — there is no network here, so treat any hit rate in my console output as a smoke test, not a result.
