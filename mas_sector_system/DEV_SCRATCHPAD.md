...
- Notes / Handoff for next agent: The user's requested filename (`pdf_generator.py`) superseded the queue's proposed `export_pdf.py`/graph-node shape. No `main.py`, `state.py`, export terminal, cost finalization, or memory path was changed; PDF-04/PDF-05 remain intentionally open. Unicode uses explicit TrueType regular/bold/italic/bold-italic faces: DejaVu Sans on Linux or a configured `MAS_PDF_FONT_DIR`, Arial on macOS/Windows. No Latin-1 sanitizer is used, and absence of a TTF family fails with an actionable error. `python3 -m py_compile mas_sector_system/pdf_generator.py` succeeds; `python3 -m pytest tests/ -q` reports 53 passed. A generated NVDA cover was rendered with Quick Look and visually inspected after correcting card alignment.
- Status: COMPLETED
