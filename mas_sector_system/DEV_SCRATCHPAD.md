...
| VAL-04 | Exemplar library (§11) → `exemplars/`. Extract reasoning moves; **never paste source memos raw**. Filter the §11.3 patterns. Note §11.5: NVDA/QCOM key to `general`. | Gemini | **REWORK** — API conforms, library thin: 3 exemplars / 2 archetypes, only 3 of the 5 §11.2 moves, and two exemplars carry figures absent from the source memos (see handoff log) |
