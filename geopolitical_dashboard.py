...
ASSETS = {
    "Commodities": {
        ...
        "Wheat": "ZW=F", "Corn": "ZC=F", "Copper": "HG=F", "Uranium (Sprott)": "SRUUF",
    },
    "Defense & Energy Stocks": {
        "Lockheed Martin": "LMT", "Raytheon (RTX)": "RTX", "Northrop Grumman": "NOC",
        "General Dynamics": "GD", "L3Harris": "LHX", "Boeing": "BA",
        "ExxonMobil": "XOM", "Chevron": "CVX", "ConocoPhillips": "COP",
        "Halliburton": "HAL", "Schlumberger": "SLB", "Occidental Petroleum": "OXY",
    },
    "Country Indices": {
        "S&P 500 (USA)": "^GSPC", "NASDAQ (USA)": "^IXIC", "Dow Jones (USA)": "^DJI",
        "FTSE 100 (UK)": "^FTSE", "DAX (Germany)": "^GDAXI", "CAC 40 (France)": "^FCHI",
        "Nikkei 225 (Japan)": "^N225", "Shanghai (China)": "000001.SS",
        "SENSEX (India)": "^BSESN", "TA-35 (Israel)": "^TA125.TA",
        "Tadawul (Saudi)": "^TASI.SR", "ADX (UAE)": "^ADI",
        "Istanbul (Turkey)": "XU100.IS", "EGX 30 (Egypt)": "^EGX30.CA",
    },
    "Sector ETFs": {
        "Energy (XLE)": "XLE", "Defense (ITA)": "ITA", "Aerospace (PPA)": "PPA",
        "Cybersecurity (CIBR)": "CIBR", "Utilities (XLU)": "XLU", "Financials (XLF)": "XLF",
        "Tech (XLK)": "XLK", "Consumer Staples": "XLP", "Materials (XLB)": "XLB",
        "Industrials (XLI)": "XLI", "Airlines (JETS)": "JETS", "Shipping (SIA)": "SIA",
    },
    "Sovereign Bonds & Safe Havens": {
        "US 10Y Treasury": "^TNX", "US 30Y Treasury": "^TYX", "US 5Y Treasury": "^FVX",
        "US 2Y Treasury": "2YY=F", "Germany 10Y Bund": "BUND-10Y.DE",
        "Gold ETF (GLD)": "GLD", "Silver ETF (SLV)": "SLV",
        "Bitcoin": "BTC-USD", "VIX (Fear Index)": "^VIX", "MOVE Index ETF": "IVOL",
    },
    "Currencies": {
        "USD Index (DXY)": "DX-Y.NYB", "EUR/USD": "EURUSD=X", "GBP/USD": "GBPUSD=X",
        "USD/JPY": "JPY=X", "USD/CHF": "CHF=X", "USD/TRY": "TRY=X", "USD/ILS": "ILS=X",
        "USD/SAR": "SAR=X", "USD/AED": "AED=X", "USD/INR": "INR=X",
        "USD/CNY": "CNY=X", "USD/RUB": "RUB=X", "USD/EGP": "EGP=X",
        "XAU/USD (Gold)": "XAUUSD=X",
    },
}

# ─── Styling (your original + small improvements) ──────────────────────────
st.markdown("""
<style>
...
def render_metric_card(name, data):
    ...
    return f'<div class="metric-card {direction}"><div style="font-size:0.85rem;color:#aaa;margin-bottom:2px;">{name}</div><div style="display:flex;justify-content:space-between;align-items:baseline;"><span style="font-size:1.2rem;font-weight:700;color:#fff;">{price:,.2f}</span><span style="font-size:0.9rem;color:{change_color};">{arrow}{change:,.2f} ({arrow}{pct:.2f}%)</span></div></div>'

def make_sparkline(history, name):
    fig = go.Figure()
    if history["Close"].iloc[-1] >= history["Close"].iloc[0]:
        color = "#00C853"; fill_color = "rgba(0, 200, 83, 0.1)"
    else:
        color = "#FF1744"; fill_color = "rgba(255, 23, 68, 0.1)"
    fig.add_trace(go.Scatter(x=history.index, y=history["Close"], mode="lines", line=dict(color=color, width=2), fill="tozeroy", fillcolor=fill_color, name=name))
...
def make_category_chart(category_data, title):
    ...
    fig.update_layout(title=dict(text=title, font=dict(size=16, color="#e0e0e0")), height=max(300, len(names)*35), margin=dict(l=10,r=40,t=40,b=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", xaxis=dict(title="% Change", gridcolor="#333", zerolinecolor="#555", color="#aaa"), yaxis=dict(autorange="reversed", color="#aaa"), font=dict(color="#ccc"))
    return fig
...
def make_heatmap(all_data):
    ...
    for cat in df["Category"].unique():
        ...
        fig.add_trace(go.Bar(name=cat, x=sub["Asset"], y=sub["Change %"], marker_color=["#00C853" if v >= 0 else "#FF1744" for v in sub["Change %"]], text=[f"{v:+.2f}%" for v in sub["Change %"]], textposition="outside", textfont=dict(size=9)))
    fig.update_layout(title="All Assets % Change Overview", barmode="group", height=500, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", xaxis=dict(tickangle=-45, color="#aaa", gridcolor="#222"), yaxis=dict(title="% Change", color="#aaa", gridcolor="#333", zerolinecolor="#555"), font=dict(color="#ccc", size=10), legend=dict(orientation="h", y=1.12), margin=dict(b=120))
    return fig
...
def get_top_movers(all_data, n=10):
    ...
    for cat, items in all_data.items():
        for name, data in items.items():
            movers.append({"Asset": name, "Category": cat, "Price": data["price"], "Change": data["change"], "Change %": data["pct_change"]})
...
# ─── NEW: Strict Iran-USA War News ─────────────────────────────────────────
@st.cache_data(ttl=300)
def fetch_iran_usa_news(api_key):
    if not api_key or len(api_key) < 10: return []
    url = "https://newsapi.org/v2/everything?q=(Iran+OR+Tehran)+(USA+OR+US+OR+Washington)+(war+OR+attack+OR+tension+OR+strike+OR+missile+OR+Hormuz+OR+conflict)&sortBy=publishedAt&language=en&pageSize=9&apiKey=" + api_key
    try:
        r = requests.get(url, timeout=10)
        return r.json().get("articles", [])[:9] if r.status_code == 200 else []
    except:
        return []
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### API Keys for Extra Features")
    news_api_key = st.text_input("NewsAPI.org Key", type="password", help="Get free key at https://newsapi.org")
...
with tab_news:
    ...
    if not news_api_key:
        st.info("👉 Enter your free NewsAPI.org key in the sidebar to load live news.")
    else:
        articles = fetch_iran_usa_news(news_api_key)
        if articles:
            cols = st.columns(3)
            for i, art in enumerate(articles):
                with cols[i % 3]:
                    st.markdown(f"""
                    <div class="news-card">
                        <div style="font-size:0.85rem;color:#FF1744;">{art['source']['name']} • {art['publishedAt'][:10]}</div>
                        <strong style="font-size:1.1rem;">{art['title']}</strong>
                        <div style="font-size:0.9rem;color:#ddd;margin-top:8px;line-height:1.4;">{art.get('description','')}</div>
                        <a href="{art['url']}" target="_blank" style="color:#00C853;margin-top:10px;display:inline-block;">Read full story →</a>
                    </div>
                    """, unsafe_allow_html=True)
        else:
            st.warning("No recent Iran-war news found or invalid API key.")
...
with tab_risk:
    ...
    with risk_col1:
        if oil_chg is not None:
            st.metric("Oil Stress", f"{oil_chg:+.2f}%", "Elevated" if abs(oil_chg) > 2 else "Normal", delta_color="inverse")
        else:
            st.metric("Oil Stress", "N/A")
    with risk_col2:
        if gold_chg is not None:
            st.metric("Gold Flight", f"{gold_chg:+.2f}%", "Safe Haven Active" if gold_chg > 0.5 else "Calm", delta_color="normal")
        else:
            st.metric("Gold Flight", "N/A")
    with risk_col3:
        if vix_price is not None:
            level = "Extreme Fear" if vix_price > 30 else "Elevated" if vix_price > 20 else "Low"
            st.metric("VIX Fear Level", f"{vix_price:.1f}", level, delta_color="inverse")
        else:
            st.metric("VIX Fear Level", "N/A")
    with risk_col4:
        if dxy_chg is not None:
            st.metric("USD Strength", f"{dxy_chg:+.2f}%", "Dollar Rally" if dxy_chg > 0.3 else "Weakening" if dxy_chg < -0.3 else "Stable")
        else:
    ...
    if len(corr_data) >= 2:
        ...
        fig_corr.update_layout(title="5-Day Return Correlations", height=450, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#ccc"), margin=dict(l=10, r=10, t=40, b=10))
